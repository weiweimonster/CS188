    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        highest_beliefs = [beliefs.argMax() for beliefs in livingGhostPositionDistributions]
        closest_p = min(highest_beliefs)
        best_action = None
        min_dist = float("inf")
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            dist = self.distancer.getDistance(successorPosition, closest_p)
            if dist < min_dist:
                best_action = action
                min_dist = dist
        return best_action